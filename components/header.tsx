    ...
    <header className="z-[999] relative">
      ...
      <nav className="flex fixed top-[0.15rem] left-1/2 h-12 -translate-x-1/2 py-2 sm:top-[1.7rem] sm:h-[initial] sm:py-0">
        <ul className="flex w-[22rem] flex-wrap items-center justify-center gap-y-2 text-[0.9rem] font-medium text-gray-500 sm:w-[initial] sm:flex-nowrap sm:gap-5">
          {links.map((link) => (
            <motion.li
              className="h-3/4 flex items-center justify-center p-2 sm:p-0"
              key={link.hash}
              ...
              <Link
                className={clsx(
                  'flex w-full items-center justify-center p-3 hover:text-slate-950 transition dark:text-slate-500 dark:hover:text-slate-300"',
                  {
                    ...
                    'flex justify-center items-center bg-slate-100 rounded-full py-1 px-2 transition duration-300 ease-in dark:bg-slate-800':
                      activeSection === link.name,
                  }
                )}
                href={link.hash}
